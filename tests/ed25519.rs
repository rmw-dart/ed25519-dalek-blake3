...
/*
  ...
  fn ed25519ph_rf8032_test_vector() {
    ...
    let mut prehash_for_signing: Hasher = Hasher::default();
    let mut prehash_for_verifying: Hasher = Hasher::default();
...
mod integrations {
  ...
  fn ed25519ph_sign_verify() {
    ...
    // ugh… there's no `impl Copy for Hasher`… i hope we can all agree these are the same hashes
    let mut prehashed_good1: Hasher = Hasher::default();
    prehashed_good1.update(good);
    let mut prehashed_good2: Hasher = Hasher::default();
    prehashed_good2.update(good);
    let mut prehashed_good3: Hasher = Hasher::default();
    prehashed_good3.update(good);

    let mut prehashed_bad1: Hasher = Hasher::default();
    prehashed_bad1.update(bad);
    let mut prehashed_bad2: Hasher = Hasher::default();
...
mod serialisation {
  ...
  static PUBLIC_KEY_BYTES: [u8; PUBLIC_KEY_LENGTH] = [
    130, 039, 155, 015, 062, 076, 188, 063, 124, 122, 026, 251, 233, 253, 225, 220, 014, 041, 166,
    120, 108, 035, 254, 077, 160, 083, 172, 058, 219, 042, 086, 120,
  ];

  static SECRET_KEY_BYTES: [u8; SECRET_KEY_LENGTH] = [
    062, 070, 027, 163, 092, 182, 011, 003, 077, 234, 098, 004, 011, 127, 079, 228, 243, 187, 150,
    073, 201, 137, 076, 022, 085, 251, 152, 002, 241, 042, 072, 054,
  ];

  /// Signature with the above keypair of a blank message.
  static SIGNATURE_BYTES: [u8; SIGNATURE_LENGTH] = [
    010, 126, 151, 143, 157, 064, 047, 001, 196, 140, 179, 058, 226, 152, 018, 102, 160, 123, 080,
    016, 210, 086, 196, 028, 053, 231, 012, 157, 169, 019, 158, 063, 045, 154, 238, 007, 053, 185,
    227, 229, 079, 108, 213, 080, 124, 252, 084, 167, 216, 085, 134, 144, 129, 149, 041, 081, 063,
    120, 126, 100, 092, 059, 050, 011,
  ];

  static KEYPAIR_BYTES: [u8; KEYPAIR_LENGTH] = [
    239, 085, 017, 235, 167, 103, 034, 062, 007, 010, 032, 146, 113, 039, 096, 174, 003, 219, 232,
    166, 240, 121, 167, 013, 098, 238, 122, 116, 193, 114, 215, 213, 175, 181, 075, 166, 224, 164,
    140, 146, 053, 120, 010, 037, 104, 094, 136, 225, 249, 102, 171, 160, 097, 132, 015, 071, 035,
    056, 000, 074, 130, 168, 225, 071,
  ];
